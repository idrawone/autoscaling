...
func (c *ExecutorCoreWithClients) trySpawnPluginRequest(
	...
	iface := c.clients.Plugin.GetHandle()
	if iface == nil {
		panic(nilHandleErr("plugin", "Plugin.GetHandle()"))
	}

	gen := iface.Generation()

	if oldGen := ongoing.plugin; oldGen != nil && *oldGen == gen {
		panic(ongoingRequestErr("plugin"))
	}
	...
	spawn(ctx, logger, "plugin", func(ctx context.Context, logger *zap.Logger) {
		resp, err := iface.Request(ctx, logger, action.LastPermit, action.Target, action.Metrics)
		endTime := time.Now()

		c.update(func(state *core.State) {
			unchanged := c.clients.Plugin.CurrentGeneration() == gen
			logFields := []zap.Field{
